...
public class CombatAssistant{
	...
	public void delayedHit(int i) { // npc hit delay
		if (Server.npcHandler.npcs[i] != null) {
			...
			if(c.projectileStage == 0 && !c.usingMagic && !c.castingMagic) { // melee hit damage
				int damageOnNPC = Misc.random(calculateMeleeMaxHit());
				c.clawDamage[0] = (int)Math.round(damageOnNPC / 2);
				c.clawDamage[1] = (int)Math.round(damageOnNPC / 4);
				c.clawDamage[2] = (int)Math.round(damageOnNPC / 8);
				applyNpcMeleeDamage(i, 1, damageOnNPC);
				if(c.doubleHit && !c.usingClaws) {
					applyNpcMeleeDamage(i, 2, Misc.random(calculateMeleeMaxHit()));
				}
				if(c.doubleHit && c.usingClaws) {
					applyNpcMeleeDamage(i, 2, c.clawDamage[0]);
					c.clawNPCDelay = 2;
					c.usingClaws = false;
				}
			}
			if(!c.castingMagic && c.projectileStage > 0) { // range hit damage
				...
				if (c.usingMorrJavs) {
					c.usingMorrJavs = false;
					Server.npcHandler.npcs[i].morrJavEffect += damage;
					Server.npcHandler.npcs[i].morrJavTimer = 3;
				}
	...
	**/
		...
		if (Server.playerHandler.players[i] != null) {
			...
			if (c.getPA().getWearingAmount() < 5 && c.duelStatus < 1) {
				c.sendMessage("You must be wearing at least 5 items to attack someone.");
				resetPlayerAttack();
				return;
                       }
	...
	public void appendVengeance(int otherPlayer, int damage) {
		...
		o.forcedText = "Taste vengeance Bitch!";
	...
	public void playerDelayedHit(int i) {
		if (Server.playerHandler.players[i] != null) {
			...
			if(c.projectileStage == 0 && !c.usingMagic && !c.castingMagic) { // melee hit damage
					int damageOnPlayer = Misc.random(calculateMeleeMaxHit());
					c.clawDamage[0] = (int)Math.round(damageOnPlayer / 2);
					c.clawDamage[1] = (int)Math.round(damageOnPlayer / 4);
					c.clawDamage[2] = (int)Math.round(damageOnPlayer / 8);							
					applyPlayerMeleeDamage(i, 1, damageOnPlayer);
					if(c.doubleHit && !c.usingClaws) {
			...
			if(!c.castingMagic && c.projectileStage > 0) { // range hit damage
				...
				if (damage > 0 && Misc.random(5) == 1 && c.lastArrowUsed == 9244) {
					if (c.lastWeaponUsed == 9185) {//This 'if statement' is the fix
						damage *= 1.45;
						o.gfx0(756);
					}
				}
				...
				if (cl2.playerEquipment[5] == 13742) {//Elysian Effect
					if (Misc.random(100) < 75) {
						...
						damages = damage2;
						damageDeduction = ((double)damages)/((double)4);
						damage2 = damage2-((int)Math.round(damageDeduction));
					}
				}
				if (cl2.playerEquipment[5] == 13740) {//Divine Effect
				    ...
				    if (damage2 > 0) {
					damages2 = damage2;
					prayer = cl2.playerLevel[5];
					possibleDamageDeduction = ((double)damages2)/((double)3.33);//30% of Damage Inflicted
					actualDamageDeduction = 0;
					if ((prayer * 2) < possibleDamageDeduction) {
					actualDamageDeduction = (prayer * 2);//Partial Effect(Not enough prayer points)
					} else {
					actualDamageDeduction = possibleDamageDeduction;//Full effect
					}
					prayerDeduction = ((double)actualDamageDeduction)/((double)2);//Half of the damage deducted
					damage2 = damage2-((int)Math.round(actualDamageDeduction));
					cl2.playerLevel[5] = cl2.playerLevel[5]-((int)Math.round(prayerDeduction));
					cl2.getPA().refreshSkill(5);
				    }
				}
				...
				if (c.usingMorrJavs) {
					c.sendMessage("Morrigan's Javelin Spec");
					Client other = (Client) Server.playerHandler.players[i];
					other.morrJavEffect += damage;
					other.morrJavTimer = 3;
					c.usingMorrJavs = false;
				}
				Server.playerHandler.players[i].underAttackBy = c.playerId;
				Server.playerHandler.players[i].logoutDelay = System.currentTimeMillis();
		...
		if (Server.playerHandler.players[playerId] != null) {
			...
			if (checkMultiBarrageReqs(playerId)) {
				...
				if (Misc.random(mageAtk()) > Misc.random(mageDef()) && !c.magicFailed) {
					...
					if (c2.playerEquipment[5] == 13742) {//Elysian Effect
						if (Misc.random(100) < 75) {
							double damages = damage;
							double damageDeduction = ((double)damages)/((double)4);
							damage = damage-((int)Math.round(damageDeduction));
					...
					if (c2.playerEquipment[5] == 13740) {//Divine Effect
						double damages2 = damage;
						double prayer = c2.playerLevel[5];
						double possibleDamageDeduction = ((double)damages2)/((double)3.33);//30% of Damage Inflicted
						double actualDamageDeduction;
						if ((prayer * 2) < possibleDamageDeduction) {
						actualDamageDeduction = (prayer * 2);//Partial Effect(Not enough prayer points)
						...
						double prayerDeduction = ((double)actualDamageDeduction)/((double)2);//Half of the damage deducted
						damage = damage-((int)Math.round(actualDamageDeduction));
						c2.playerLevel[5] = c2.playerLevel[5]-((int)Math.round(prayerDeduction));
	...
	**/
	...
	public void activateSpecial(int weapon, int i){
		...
		switch(weapon) {
			...
			c.specDamage = 1.34;
			c.specAccuracy = 3.3;
			c.forcedText = "Here's the BanHammer!";
			c.forcedChatUpdateRequired = true;
			c.updateRequired = true;
			break;
			...
			case 13879://Morr jav
			c.usingBow = true;
			c.usingMorrJavs = true;
			c.lastWeaponUsed = weapon;
			c.hitDelay = 3;
			c.startAnimation(806);
			c.projectileStage = 1;
			c.gfx100(getRangeStartGFX());
			c.hitDelay = getHitDelay(c.getItems().getItemName(c.playerEquipment[c.playerWeapon]).toLowerCase());
			if (c.fightMode == 2)
	...
	**/
	...
	public int calculateMeleeDefence()
    {
        ...
        if (c.prayerActive[0]) {
            ...
            defenceLevel += c.getLevelForXP(c.playerXP[c.playerDefence]) * 0.32;
        }
	...
	**/
	...
	public void applyRecoil(int damage, int i) {
		...
		if (damage > 0 && Server.playerHandler.players[i].playerEquipment[c.playerRing] == 773) {
			int recDamage = damage/10 + 1;
			if (!c.getHitUpdateRequired()) {
				c.setHitDiff(recDamage);
				c.setHitUpdateRequired(true);				
