...
public class PlayerAssistant{
		...
		switch(spellId) {
			...
			if(System.currentTimeMillis() - c.alchDelay > 1000) {	
				...
				for (int i = 0; i < Config.ITEM_TRADEABLE.length; i++) {
					if (Config.ITEM_TRADEABLE[i] == itemId) {
						c.sendMessage("You can't alch this item");
						return;
					}
				}
	...
	**/
	...
	public void startTeleport(int x, int y, int height, String teleportType) {
		...
		if(c.inPits) {//Glitchfix by mikee
			c.sendMessage("You cannot teleport from the Fight Pits.");
			return;
		}
		if(c.getPA().inPitsWait()) {//Glitchfix by mikee
			c.sendMessage("You cannot teleport from the Fight Pits waiting room.");
			return;
		}
	...
	**/
	...
	public void followPlayer() {
		if(Server.playerHandler.players[c.followId] == null || Server.playerHandler.players[c.followId].isDead || Server.playerHandler.players[c.followId].hidden) {
			c.followId = 0;
			return;
		}		
		if(c.freezeTimer > 0) {
