...
break;
			...
			if (!c.getCombat().checkSpecAmount(c.playerEquipment[c.playerWeapon])) return;
				if (!c.usingSpecial) {
					c.usingSpecial = true;
					Client o = null;
					if (Server.playerHandler.players[c.playerId].underAttackBy != 0) {
						o = (Client) Server.playerHandler.players[Server.playerHandler.players[c.playerId].underAttackBy];
						if (c.goodDistance(o.getX(), o.getY(), c.getX(), c.getY(), 20)) {
						c.playerIndex = Server.playerHandler.players[c.playerId].underAttackBy;
						c.getCombat().attackPlayer(Server.playerHandler.players[c.playerId].underAttackBy);
						c.attackTimer = 0;
						c.specAmount -= 5;
						}
					} else {
						o = (Client) Server.playerHandler.players[c.lastAttacked];
						if (c.goodDistance(o.getX(), o.getY(), c.getX(), c.getY(), 20)) {
						c.playerIndex = c.lastAttacked;
						c.getCombat().attackPlayer(Server.playerHandler.players[c.playerId].underAttackBy);
						c.attackTimer = 0;
						c.specAmount -= 5;
						}
