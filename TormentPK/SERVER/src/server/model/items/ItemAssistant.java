...
public class ItemAssistant {
	...
	**/
	...
	public void dropAllItems() {
		...
		if (o != null) {
			try {
				BufferedWriter bw = new BufferedWriter(new FileWriter("./Data/logs/kills.txt", true));
				bw.write(o.playerName+" has gotten the drop for killing "+c.playerName);
            			bw.newLine();
            			bw.close();
			} catch (Exception e) {}
		}
	...
	**/
	...
	public void sendWeapon(int Weapon, String WeaponName) {
		...
		} else if (WeaponName.equalsIgnoreCase("Morrigan's javelin") || WeaponName.equalsIgnoreCase("Morrigan's thrown axe") || WeaponName.endsWith("bow") || WeaponName.endsWith("10")|| WeaponName.endsWith("full") || WeaponName.startsWith("seercull")) {
	...
	**/
	...
	public void addSpecialBar(int weapon) {
		switch(weapon) {
			...
			c.getPA().sendFrame171(1, 7649);  // jav/thrownaxe interface
	...
	public boolean wearItem(int wearID, int slot) {
		synchronized(c) {
			...
			if(c.playerItems[slot] == (wearID+1)) {				
				...
				if ((wearID == 15000 || wearID == 13899 || wearID == 13902 || wearID >= 13738 && wearID <= 13744) && (c.memberStatus < 1)) {
					...
					if (toEquip == toRemove + 1 && Item.itemStackable[toRemove]) {
						...
						if (targetSlot == 13) {
							if (toRemove >= 0 &&
									...
									&& getItemAmount(toRemove) > 0) {
								dupeFix += getItemAmount(toRemove);
								deleteItem(toRemove, getItemSlot(toRemove), getItemAmount(toRemove));
							}
						}
						c.playerItems[slot] = toRemove + 1;
						c.playerItemsN[slot] = toRemoveN;
						c.playerItemsN[slot] += dupeFix;
	...
	*/
	...
	public boolean bankItem(int itemID, int fromSlot, int amount){
		if (!c.isBanking) {
			c.sendMessage("You have been detected as trying to abuse a glitch!");
			c.sendMessage("This has been logged and u will most likely be banned :C");
			return false ;
		}
		if(c.inTrade) {
